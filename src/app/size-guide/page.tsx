...
export const metadata: Metadata = {
  ...
  description: "Find the perfect fit with our detailed size chart for boys, girls, men, and women.",
};
    ...
    <div className="min-h-screen bg-ivory">
      ...
      <section className="py-16 sm:py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          {(["boy", "girl", "men", "women"] as const).map((category) => (
            <div key={category}>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-ivory border-b border-border">
                        ...
                        {(category === "boy" || category === "girl") && <th className="text-left px-4 py-3 font-semibold text-navy">Age</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {SIZE_CHART[category].map((size, i) => (
                        <tr key={size.label} className={i % 2 === 0 ? "" : "bg-ivory/50"}>
                          ...
                          {(category === "boy" || category === "girl") && <td className="px-4 py-3 text-warm-grey">{size.label}</td>}
                        </tr>
                      ))}
                    </tbody>
                  </table>
